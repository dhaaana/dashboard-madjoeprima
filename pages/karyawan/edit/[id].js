...
export default function EditKaryawan({ data }) {
  const [formData, setFormData] = useState({});
  const router = useRouter();

  async function updateNew(e) {
    e.preventDefault();
    const response = await fetch(`/api/karyawan/update/${data.idKry}`, {
      method: "POST",
        ...
        <div className="max-w-md w-2/3 bg-white rounded p-6 space-y-4 filter drop-shadow-sm">
          ...
          <form onSubmit={updateNew}>
            ...
            <input
              ...
              placeholder="ID"
              defaultValue={data.idKry}
              name="idKry"
              onChange={(e) => setFormData({ ...formData, idKry: e.target.value })}
            />
...
export async function getServerSideProps({ params }) {
  const { id } = params;
  const bahan = await prisma.karyawan.findUnique({
    where: {
      idKry: id,
    },
  });
