...
export default function EditPO({ data, bar, kli }) {
  const [formData, setFormData] = useState({});
  const router = useRouter();

  async function updateNew(e) {
    e.preventDefault();
    const response = await fetch(`/api/po/update/${data.idPO}`, {
      method: "POST",
        ...
        <div className="max-w-md w-2/3 bg-white rounded p-6 my-4 space-y-4 filter drop-shadow-sm">
          ...
          <form onSubmit={updateNew}>
            ...
            <input
              ...
              placeholder="ID"
              defaultValue={data.idPO}
              name="idPO"
              onChange={(e) => setFormData({ ...formData, idPO: e.target.value })}
            />
            <label htmlFor="idBar" className="block text-sm font-medium text-gray-700 my-2">
              ID Barang
            </label>
            <select required className="w-full p-4 text-sm bg-gray-50 focus:outline-none border border-gray-200 rounded text-gray-600" defaultValue={data.idBar} name="idBar" onChange={(e) => setFormData({ ...formData, idBar: e.target.value })}>
            ...
            <select required className="w-full p-4 text-sm bg-gray-50 focus:outline-none border border-gray-200 rounded text-gray-600" defaultValue={data.idKli} name="idKli" onChange={(e) => setFormData({ ...formData, idKli: e.target.value })}>
              <option value={0}>Pilih salah satu</option>
              {kli.map((item) => {
                return (
...
export async function getServerSideProps({ params }) {
  ...
  const data = await JSON.parse(JSON.stringify(po));
